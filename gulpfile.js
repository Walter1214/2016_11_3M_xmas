...
gulp.task('m', () => {
    log('m');
    var IMG_SRC = ['src/img_src/**/*.+(jpg|png|gif)', '!src/img_src/_*'];
    var OTHER_IMG = ['!src/img_src/**/*.(jpg|png|gif)', 'src/img_src/_*'];
    var ICO_IMG = ['src/img_src/**/*.ico'];
    var DIST = 'src/img';
    ...
    var taskICOIMG = gulp.src(ICO_IMG)
        .pipe($.changed(DIST))
        .pipe($.size({
            title: '',
            showFiles: true
    ...
    return merge(taskOtherIMG, taskICOIMG, taskIMGSRC);
});
